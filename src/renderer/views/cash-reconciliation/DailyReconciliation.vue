<template>
  <v-card>
    ...
    <v-card-text>
      ...
      <div v-if="reconciliation">
        <h3 class="mb-4">Ventas del Día</h3>
        <v-data-table
          :headers="salesHeaders"
          :items="todaysSalesItems"
          :loading="isLoading"
          item-key="id"
          class="elevation-1"
        >
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn size="small" variant="text" color="success" @click="viewInvoice(item)">
              <v-icon>mdi-file-document</v-icon>
              <v-tooltip activator="parent">Ver Factura</v-tooltip>
            </v-btn>
          </template>
        </v-data-table>
      </div>
      ...
      <!-- Invoice Dialog -->
      <v-dialog v-model="showInvoiceDialog" max-width="800px">
          ...
          <v-card-text class="pa-6">
            <div id="invoice-print" class="invoice-print">
              ...
              <div class="text-center mb-6">
                ...
                <div class="mt-4">
                  <p class="text-h6 font-weight-bold">Factura #{{ selectedSale.saleNumber }}</p>
                  <p class="text-body-2">Fecha: {{ formatDate(selectedSale.sale_date) }}</p>
                </div>
              </div>
              ...
              <v-row class="mb-4">
                <v-col cols="6">
                  ...
                  <div>{{ selectedSale.user?.firstName }} {{ selectedSale.user?.lastName }}</div>
                </v-col>
                <v-col cols="6">
              ...
              <div class="mb-4">
                ...
                <table class="product-table border" style="width: 100%">
                  <thead>
                    <tr>
                  ...
                  <tfoot>
                    <tr class="total-row">
                      ...
                      <td style="text-align: end"><strong>{{ formatCurrency(selectedSale.totalUsd, 'USD') }}</strong></td>
                      <td style="text-align: end"><strong>{{ formatCurrency(selectedSale.totalBs, 'VES') }}</strong></td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <!-- Métodos de pago -->
              <div class="mb-4" v-if="selectedSale.payments && selectedSale.payments.length > 0">
                <div class="text-subtitle-1 font-weight-bold mb-2">Métodos de Pago</div>
                <table class="payment-table border" style="width: 100%">
                  <thead>
                    <tr>
                ...
                <v-col cols="4">
                  <v-card variant="outlined" class="pa-3">
                    <div class="d-flex justify-space-between mb-2">
                      ...
                      <span>{{ formatCurrency(selectedSale.totalUsd, 'USD') }}</span>
                    </div>
                    <div class="d-flex justify-space-between mb-2">
                      <span class="font-weight-medium">Total Bs:</span>
                      <span class="font-weight-bold">{{ formatCurrency(selectedSale.totalBs, 'VES') }}</span>
                    </div>
                    <v-divider class="my-2"></v-divider>
                    <div class="d-flex justify-space-between">
                      <span class="font-weight-bold text-primary">TOTAL PAGADO:</span>
                      <span class="font-weight-bold text-primary">{{ formatCurrency(selectedSale.totalBs, 'VES') }}</span>
                    </div>
                  </v-card>
                </v-col>
              </v-row>
              ...
              <!-- Notas -->
              <div v-if="selectedSale.notes" class="mt-4">
                <div class="text-subtitle-2 font-weight-bold">Notas:</div>
                <div class="text-body-2">{{ selectedSale.notes }}</div>
              </div>
            </div>
          </v-card-text>
          <v-card-actions class="pa-4 bg-grey-lighten-4">
            <v-spacer></v-spacer>
            <v-btn color="primary" variant="outlined" @click="printInvoice">
              <v-icon class="mr-2">mdi-printer</v-icon>
              Imprimir
            </v-btn>
            <v-btn @click="showInvoiceDialog = false">Cerrar</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
